...
export function createApp() {
  ...
  app.use((error: unknown, request: Request, response: Response, _next: NextFunction) => {
    ...
    if (isTransientDbError(error)) {
      request.log.warn({ err: error }, 'Conexão transitória com banco encerrada (E57P01). Reconexão agendada.');
      prisma.$disconnect().catch(() => {});
      response.status(503).json({
        error: {
          code: 'DATABASE_RECONNECTING',
          message: 'O serviço de banco de dados está restabelecendo conexão. Tente novamente em instantes.',
          requestId: request.id,
        },
      });
      return;
